...
extension DirectDictationController {
  struct Dependencies {
    ...
    // Text cleanup and correction learning (this fork).
    let setVocabulary: @Sendable ([String]) async -> Void
    /// Reads the target field back so CorrectionWatcher can notice an edit.
    /// A closure rather than the service itself, so the watcher never gains a
    /// route to write.
    let readFieldValue: @MainActor (TextInsertionService.Target) -> String?
    let showCleaning: @MainActor () -> Void
    ...
    static func live(
      ...
      return Self(
        ...
        readFieldValue: { textInsertionService.readValue(of: $0) },
        showCleaning: { hudController.showCleaning() },
